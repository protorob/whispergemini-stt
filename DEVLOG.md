...
- [x] **Speaker diarization — live-verified + 4 pyannote version-compat fixes, ETA + layout follow-up** (2026-08-04, same day as the entry below): confirmed working end-to-end on a real Windows machine (RTX 3070 Ti laptop, real `HF_TOKEN` with the license accepted) after fixing four real issues the dev sandbox couldn't have caught without `pyannote.audio` actually installed: (1) `torchcodec` (pyannote's default file-path audio decoder) failed to load its Windows DLLs — already sidestepped functionally since `_load_waveform()` reads the ffmpeg-normalized WAV via stdlib `wave` and hands pyannote an in-memory tensor instead of a file path, but the import-time warning itself needed a `catch_warnings()` block scoped tightly around just the `pyannote.audio` import (`_import_pyannote_quietly()`) — a plain module-level `warnings.filterwarnings()` was tried first and did NOT reliably suppress it, something later in pyannote/torch's own import chain resets the global filter list; (2) the installed `pyannote.audio`'s `Pipeline.from_pretrained()` no longer takes `use_auth_token=` (renamed to `token=`, matching `huggingface_hub`'s own rename) — `_get_pipeline()` now tries `token=` first, falls back to `use_auth_token=` on `TypeError`; (3) the installed version's `pipeline()` call returns a `DiarizeOutput` wrapper instead of a `pyannote.core.Annotation`, breaking `.itertracks()` — `_extract_tracks()` tries `.itertracks()` then a few guessed wrapper attribute names, and raises a diagnostic listing the object's real attributes if none match (which of the guesses actually fired was never confirmed — the user just reported it working); (4) the transcription ETA didn't know diarization adds a second full model pass, so multi-speaker runs sat on "running longer than estimated" for most of their duration — fixed by giving multi-speaker runs their own RTF calibration bucket (`app.js`'s `rtfKey()`, single-speaker keeps its original key so old calibration isn't lost) plus a flat seed surcharge until real data exists for that combo. Also, separately reported as too cramped once diarization overlays were visible: widened the page from `40rem` to `56rem` max-width (`style.css`). All four fixes plus the two follow-ups are on `main` (commits `e68f22b`..`3e0dbd9`).

  One more benign warning seen in real use, worth noting so it isn't
  mistaken for a bug later: `pyannote/audio/models/blocks/pooling.py:103:
  UserWarning: std(): degrees of freedom is <= 0 ...`. This comes from
  pyannote's speaker-embedding model's internal statistics-pooling layer —
  it computes a sample standard deviation (needs ≥2 data points) over a
  window of audio frames, and this fires when some window (typically a
  short trailing chunk near the end of the clip, or a brief isolated
  speech segment) ends up with just one frame. It's purely internal model
  math, not something our code triggers or can meaningfully avoid, and it
  didn't stop diarization from completing correctly during real testing —
  cosmetic log noise, no action needed.

  **Confirmed by design, not just observed:** "Multiple speakers" is a
  genuinely separate pipeline, not just a labeling difference on the same
  run — diarization is a full second model pass (pyannote's segmentation +
  embedding + clustering) over the same normalized WAV, run in addition to
  (not instead of) faster-whisper/Parakeet's own pass. That's the whole
  reason the ETA needed its own calibration bucket (fix 4 above) rather
  than reusing single-speaker's numbers.
...
- [x] **Rebrand to Cassiodorus + light/dark theme + About popup + client-side session persistence** (2026-08-04): the app formerly known as `am-whisper-stt` is now **Cassiodorus** — *Ancient diligence. Modern intelligence.* — renamed across `frontend/index.html`'s `<title>`/`<h1>`, the FastAPI `title=`, and `README.md`/`PLAN.md`. See `BRAND.md` (new) for the full naming rationale, and `DEPLOYMENT.md` (new) for a separate discussion of hosting/monetization directions (deferred, not built). Four pieces landed together:
  - **Color palette + light/dark toggle**: replaced the old flat blue `--accent` with a real day/night system adopted from [vates.standout.jp](https://vates.standout.jp/) (read directly from their live CSS, not guessed) — warm ivory/deep indigo by day, near-black navy/warm parchment/bonfire-gold by night, deliberately near-monochrome in daylight (gold is a night-only accent, matching that reference's own restraint). New `--accent-contrast` custom property fixes a real contrast bug this surfaced: `--accent` itself flips from dark (day) to light (night, gold), so a hardcoded `color: #fff` on every filled button would've been unreadable white-on-gold at night. A sun/moon toggle button (top-right, `#theme-toggle-btn`) overrides `prefers-color-scheme` via a `data-theme` attribute, persisted in `localStorage`. Also caught and fixed one hardcoded `rgba()` tint of the *old* blue accent (dropzone hover) that would've silently stayed blue forever — now `color-mix(in srgb, var(--accent) 8%, transparent)`, always theme-aware.
  - **Textarea contrast bug, found via a real screenshot**: the transcript preview showed near-invisible pale text on a gray panel in light mode — root cause was `color-scheme: light dark` on `:root` letting the browser paint the native `<textarea>` with *its own* dark-mode default text color whenever the OS prefers dark, independent of which theme the page's own `[data-theme]` override was actually showing. Fixed by giving `.preview`/`textarea.preview` explicit `color`/`background` sourced from the app's own variables rather than trusting the UA default — the actual fix, not a cosmetic tweak.
  - **About/how-to-use popup**: an info-icon button next to the theme toggle opens a native `<dialog>` (zero new dependencies) with About/How it works/How to use sections written from what's actually shipped, plus a `mailto:` contact link. The `<dialog>` box itself is deliberately bare (no padding/background) with a separate `.about-dialog-content` child carrying the visual card styling — required for click-outside-to-close (`event.target === dialog`) to only fire on genuine backdrop clicks, not clicks inside the dialog's own padding.
  - **Client-side session persistence**: audio, transcript (+edits), pause/speaker markers, and the Gemini-formatted result now survive a page reload, restored automatically. IndexedDB holds the audio blob (localStorage is string-only and far too small); localStorage holds everything else. Persists the *original* selected blob rather than a server-normalized re-encode (avoids an ffmpeg round trip on every source pick), just size-capped at 50MB — over that, text/options still persist but audio doesn't, and a session with nothing restorable is dropped cleanly. Only cleared by explicit action: Delete/close on the review card now confirms first whenever a session exists, then wipes both storage layers. Deliberately does not restore the engine/model dropdowns (populated async from `/api/capabilities`; not worth the sequencing complexity). See `DEPLOYMENT.md` for the full caveats (storage isn't guaranteed durable, private browsing gets ephemeral storage, etc.).
  - **Real bug caught during this**: the Gemini-formatted result didn't survive a reload even though audio/transcript did. Root cause was save-side, not restore-side — `persistSession()`'s `catch` block silently swallowed *any* error, not just the expected storage-full/unavailable ones, which would have hidden a genuine bug if there'd been one. Root cause turned out to be reload timing (reloading before the enhance call had actually resolved and called `persistSession()` — the transcript's own persist happens earlier, at transcribe-success, so it looks like a partial restore rather than what it is). Fixed by making the catch block `console.warn` instead of swallowing silently, and confirmed working end-to-end by the user after waiting for "Done." before reloading.
- [x] **Upload size limit** (2026-08-04): found via a real test on a second Windows machine — uploading a long uncompressed-WAV phone-call recording (over 2GB) threw an uncaught `decodeAudioData` error in `loadSourcePreview()` (browsers hard-cap that API at 2GB, regardless of machine RAM) and the actual `/api/transcribe` upload separately failed with a network-layer error before even reaching the server (a >2GB `fetch` body hits the same class of browser limit) — two different symptoms of the same root cause rather than two bugs. Rather than trying to support arbitrarily large files against a hard browser ceiling, added a real, configurable limit: `MAX_UPLOAD_MB` (`backend/app/config.py`, default 1024) is enforced in `/api/transcribe` by streaming the upload in 1MB chunks and rejecting with a clean 413 as soon as the running total crosses the limit — replacing the old `tmp_in.write(await file.read())`, which read the whole file into memory before any size check could happen. The limit is also exposed via `GET /api/capabilities` (`max_upload_mb`) so the frontend enforces the same number *before* ever touching the file: `handleFileSelected()` in `app.js` checks `file.size` up front and shows a clear message ("try a compressed format or a shorter clip") instead of letting WaveSurfer/FileReader run into the browser's 2GB wall. Documented in `.env.example` and README's Configuration table. Not yet re-verified against the original failing file — worth confirming the 413/clear-message path fires correctly on that same Windows machine.
...
Speaker diarization and the rebrand/theming/session-persistence work
(see the status entries above) are both live-verified now. The upload
size limit (see the entry above) is implemented but not yet re-verified
against the original failing file on the second Windows machine — do
that first if picking this thread back up. Two threads open next, from
the deployment/product-direction discussion in `DEPLOYMENT.md`:

1. Decide the real direction — self-hosted/local-first tool vs. a hosted
   product with a paid cloud tier — before starting either #1 (browser-side
   WASM/WebGPU transcription) or #2 (token-based billing) from that doc.
   Neither has been started; both need real architecture decisions first,
   not just code.
2. **Phase 7 — Coolify readiness** (healthcheck wiring, env-based
   overrides, model-cache volume, deployment docs) — still not started.
   See `PLAN.md` for the full phase breakdown and architecture rationale,
   the "GPU inference finding" note above before touching `hardware.py`,
   and the Parakeet "What was and wasn't tested" note above before
   trusting `Dockerfile.gpu` or `parakeet_engine.py` in production — both
   need a real run on an actual GPU host.

Smaller loose end: `BRAND.md`'s open decisions (fish-or-not in a future
logo mark, whether to self-host a real EB Garamond webfont) are still
